...
class tmhOAuth {
  ...
  private function curlit() {
    ...
    if ( ! empty($this->request_params) ) {
      ...
      // CURL will not set the content-length  (or set it to -1) when there is no data, which breaks Vimeo
      // override this and set the length to 0 in that case.
      $this->headers['Content-Type'] = '';
      $this->headers['Content-Length'] = '0';
    }

    // CURL defaults to setting this to Expect: 100-Continue which Vimeo rejects
    $this->headers['Expect'] = '';
